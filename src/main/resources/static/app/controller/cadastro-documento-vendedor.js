app.controller("cadastroDocumentoVendedorController", function ($scope, $http, messageService, oportunidadeService) {

    $scope.documento = {};
    $scope.documentos = [];
    $scope.agrupamentoDocumentos = [];
    var baseUrl = 'api/modulo-gi/doc-vendedor';

    $scope.findAll = function () {
        $scope.documentos = [];
        oportunidadeService.get('api/modulo-gi/doc-vendedor/todos').then(function (response) {
    ...
    $scope.save = function () {
        ...
        $http({
            method: 'POST',
            url: 'api/modulo-gi/doc-vendedor',
